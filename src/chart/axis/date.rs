...
/// A date axis on a chart (used when categories are date values).
#[derive(Debug, Clone)]
...
impl DateAxis {
    /// Create a new date axis with default settings.
    #[must_use]
    pub fn new() -> Self {
        Self {
            has_title: false,
...
/// Creates a date axis with no title and default tick/grid settings.
impl Default for DateAxis {
    fn default() -> Self {
        Self::new()
    }
}
